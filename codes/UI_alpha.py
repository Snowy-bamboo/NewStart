...
from Speaker import Speak,Volume
...
def Foo():
	"调试时的占位函数，最终版橄榄"
	messagebox.showwarning("该程序无功能","请不要直接运行UI.py文件\n如果你正常启动系统\n并遇到该弹窗,联系孙先生")
	return True
...
class UI_alpha:
	...
	def Init_Menu(self):
		...
		self.helpmenu.add_command(label="帮助",command=self.help,accelerator="F10")
		self.win.bind("<F10>",lambda event:self.help())
	...
	def help(self):
		Speak("赶紧去培训，不会出车的东西！")
		messagebox.showwarning("才不是作者懒得写文档","赶紧去培训，\n不会出车的东西！")
	def about(self):
...
class Left_UI:
	...
	def Loaddata(self):
		"导入出车记录"
		#获取文件路径
		if len(self.EquipmentTree.get_children()):
			Speak("警告")
			if (not messagebox.askokcancel("警告","确定删除全部器材？\n该过程无法被撤销！")):
				return
	...
	def UpdateSubTreeMenuScan(self):
		...
		self.SubTreeMenuScan.add_command(label='根目录',command=lambda g='': [self.CurrentRoot.set(g),Speak("扫描进根目录")])
		...
		for qz in CurrentAllQZ:
			self.SubTreeMenuScan.add_radiobutton(label=self.Check_Name(qz),value=qz, command=lambda g=qz: [self.CurrentRoot.set(g),Speak("扫描进"+self.Check_Name(g))],variable=self.CurrentRoot)
	...
	def TreeAdd(self,code:str):
		...
		if code in AllQZDict: #！！！这里是特指真器组而非自己创建的伪器组，所以是全局变量！！！
			#添加器组的情况，先添加根目录
			Speak(code+self.Check_Name(code))
			...
			#自定义器组的情况
			Speak("添加器组"+self.Check_Name(code))
			Appendlist.append(code)
			#添加器材至当前指定的目录
			self.EquipmentTree.insert("",tk.END,iid=code,text='',value=[self.Check_Name(code)])
	...
	def TreeMove(self,goal:str):
		...
		Speak("将"+self.Check_Name(code)+"移动到"+self.Check_Name(goal))
